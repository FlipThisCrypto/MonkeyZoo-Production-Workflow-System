...
def evidence(folder,root):
 metadata=issue_workflow._json(folder/"metadata.json") or {};exports=folder/"exports";pdfs=sorted(exports.glob("*.pdf")) if exports.exists() else [];packages=sorted([*exports.glob("*.zip"),*exports.glob("*.cbz")]) if exports.exists() else [];covers=sorted((folder/"generated_art").rglob("*cover*.png")) if (folder/"generated_art").exists() else []
 ...
 files=[folder/"metadata.json",folder/"social_posts.md",folder/"final_export_checklist.md",folder/"cover_prompt.md",folder/"qa_report.md",*covers,*pdfs,*packages];entries=[]
 for path in files:
 ...
 if not packages or not any(p.stat().st_size for p in packages):blockers.append("Final CBZ or ZIP package is missing or empty")
 ...
 return {"evidence_hash":digest.hexdigest(),"files":entries,"blockers":blockers,"qa_verdict":qa,"qa_evidence_hash":reported_hashes[0] if len(reported_hashes)==1 else None,"qa_evidence_current":bool(current_qa_hash and len(reported_hashes)==1 and reported_hashes[0]==current_qa_hash),"covers":[str(p.relative_to(folder)).replace("\\","/") for p in covers],"pdfs":[p.name for p in pdfs],"packages":[p.name for p in packages],"metadata":{"format":metadata.get("format"),"missing_fields":missing_meta,"placeholders":placeholders},"social_copy_exists":(folder/"social_posts.md").exists(),"checklist_exists":(folder/"final_export_checklist.md").exists(),"archive":{"path":str(archive.relative_to(root)).replace("\\","/"),"exists":archive.exists(),"publication_files":[p.name for p in archive.rglob("*") if p.is_file()] if archive.exists() else []}}
...
def promote_manifest(folder,root,replace=False):
 ...
 source=_workspace(folder)/"manifests"/f"{status['approval']['manifest_hash']}.json";data=_read_json(source)
 if not data:raise ReleaseError("Approved release manifest is missing",409)
 _write_json(destination,data);return {"ok":True,"destination":"release_hash_manifest.json","manifest_hash":data["manifest_hash"],"workflow":issue_workflow.workflow_status(folder,root)}
