...
def evidence(folder):
 ...
 checks={"missing_panels":[x["panel_id"] for x in inventory if not x["exists"]],"duplicate_art":duplicates,"invalid_images":[x["panel_id"] for x in inventory if x["error"]],"dimension_mismatches":[x["panel_id"] for x in inventory if x["exists"] and not x["error"] and (x["width"],x["height"])!=expected_dimensions],"identity_missing":[x["panel_id"] for x in inventory if not x["characters"]],"unmapped_images":[p.stem for p in selected.glob("*.png") if p.stem not in planned] if selected.exists() else [],"dialogue_or_caption_panels":[x["panel_id"] for x in inventory if x["dialogue"] or x["caption"]],"continuity_missing":[x["panel_id"] for x in inventory if not x["continuity_notes"]],"cover_prompt_exists":(folder/"cover_prompt.md").exists(),"cover_images":len(covers),"metadata_missing":metadata_missing,"checklist_exists":(folder/"final_export_checklist.md").exists()}
 blockers=[]
 ...
 return {"evidence_hash":digest.hexdigest(),"panels":inventory,"checks":checks,"blockers":blockers,"planned_panel_count":len(planned),"selected_panel_count":sum(x["exists"] for x in inventory)}
...
def promote(folder,root,rid,replace=False):
 ...
 with _promotion_lock(folder):
  ...
  backup=None
  if destination.exists():backup=_workspace(folder)/"promotions"/f"backup-{rid}.md";_atomic(backup,destination.read_text(encoding="utf-8"))
  _atomic(destination,_render(record));prov={"review_id":rid,"promoted_at":_now(),"evidence_hash":record["evidence_hash"],"verdict":record["verdict"],"destination":"qa_report.md","backup":str(backup.relative_to(folder)).replace("\\","/") if backup else None,"actor":"project_owner"};_write_json(provenance,prov)
  return {"ok":True,"promotion":prov,"workflow":issue_workflow.workflow_status(folder,root)}
