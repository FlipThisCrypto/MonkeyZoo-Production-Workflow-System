...
| U04 | Medium | Repair | `character-bibles/_review_app/art_prompt_workspace.py:157` | Style-lock (Rule 3) phrase has two constructions; the Studio's scrape leaks markdown blockquote gutters into every promoted panel prompt | Open |
...
| U06 | Medium | Consolidate | `character-bibles/_review_app/page_panel_workspace.py:177` | Studio promotes page plans that validate_issue.py (the Stage 4/5/9 gate) rejects: emotion and camera_angle are required by one implementation and not the other | Open |
